...
const InnerSubCategoryProductPage = () => {
  ...
  const handleEffect = async ()  =>{
    try {
      ...
      const filterProduct = res.data.data.filter((item)=>item.categoryName === category && item.subCategoryName === subcategory && item.AgainSubCategoryName === innerSubCategory);
  ...
  return (
    ...
    {product && product.map((item,index)=>(
      <div key={index}>
        <Breadcrumb title={item.categoryName} middle={{url:`/AgainSub/${item.categoryName}/${item.subCategoryName}`,text:`${item.subCategoryName}`}}  last={item.productName} />
...
export default InnerSubCategoryProductPage